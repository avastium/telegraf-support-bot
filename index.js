...
bot.on('message', (ctx) => {
  if (ctx.chat.id == process.env.GROUP_ID && reply_to_message in ctx.message) {
    db.collection('messages').findOne({msg_id: ctx.message.reply_to_message.message_id}).then((res) => {
      ctx.telegram.copyMessage(res.tg_id, process.env.GROUP_ID, ctx.message.message_id);
    }).catch(e => console.log(e));
  } else {
    ctx.telegram.forwardMessage(process.env.GROUP_ID, ctx.chat.id, ctx.message.message_id).then((res) => {
      db.collection('messages').insertOne(new Message({tg_id: ctx.chat.id, msg_id: res.message_id, time: Date.now()})).then().catch(e => console.log(e));
    }).catch(e => console.log(e));
  }
});
